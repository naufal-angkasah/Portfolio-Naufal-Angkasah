...
export const projects: Project[] = [
  // ═══ FULL STACK & WEB DEV PROJECTS WITH SCREENSHOT GALLERIES ═══
  {
    ...
    type: "Full Stack",
    category: "Full Stack",
    desc: "Sistem terpadu tata kelola klaster perumahan: Peta siteplan SVG interaktif, portal warga, sistem tagihan IPL & meteran air, gateway Midtrans, otomasi Fonnte/Nodemailer, PWA mobile native, dan isolasi dual-session anti-bentrok.",
    longDesc:
      "Aplikasi web enterprise full-stack Sistem Pengelolaan Kawasan Perumahan Kumala Cluster (Grand Tamansari / Wimala-Land) yang dibangun dengan Next.js 15.5.25 App Router (Server Components & Route Handlers), React 19.3.0, TypeScript 5, dan Tailwind CSS v4. Menghadirkan sinkronisasi 100% data warga dengan peta siteplan SVG interaktif berfitur drag-and-drop marker kavling (koordinat X% & Y%). Dilengkapi portal mandiri warga untuk tagihan terpadu (IPL, meteran air m³, denda fleksibel), pembayaran online Midtrans Snap (QRIS, VA Bank) & verifikasi transfer manual, generator invoice bulanan otomatis, broadcast selektif via WhatsApp (Fonnte API) & Email (Nodemailer SMTP), pusat pengaduan berstatus transparan, Progressive Web App (PWA) dengan custom Service Worker & fitur Double Back to Exit, isolasi dual-session (gt_admin_session & gt_warga_session) anti-bentrok multi-tab, auto-logout 5 menit inactivity timeout, antarmuka ramah orang tua (Elderly-Friendly UI), serta arsitektur dual-database real-time Google Firebase Cloud Firestore (v12.19.0) & Supabase PostgreSQL.",
    stack: [
      "Next.js 15.5 (App Router)",
      "React 19.3 & TypeScript 5",
      "Tailwind CSS v4",
    ...
    featureModules: [
      {
        ...
        features: [
          { name: "Peta SVG Interaktif Multi-Cluster", desc: "Denah vektor visual klaster (Kamala, Lily, Bougenvile) dengan indikator warna status pembayaran (Hijau = Lunas, Merah = Belum Bayar, Biru = Menunggu Verifikasi, Abu-abu = Kosong)" },
          { name: "Drag-and-Drop Titik Rumah", desc: "Mode edit memindahkan marker kavling langsung di atas denah peta SVG dengan kalkulasi koordinat posisi X% & Y% tersimpan secara real-time" },
          { name: "Paritas 100% Data Warga & Siteplan", desc: "Formulir Tambah & Edit Warga terintegrasi penuh dengan titik peta: Nama, Blok Kavling (D-01), Cluster, WhatsApp, Email, Tipe Rumah (Ebony/Angsana), Status Hunian, Status Pembayaran & Posisi Koordinat" },
          ...
          { name: "Pencarian & Multi-Filter Cepat", desc: "Pencarian nama konsumen/pemilik, nomor blok, filter per cluster perumahan, dan klasifikasi status hunian (Terhuni, Booking, Tersedia)" },
        ],
      },
      {
        icon: "💵",
        module: "Super Admin: Keuangan, Tagihan Rutin & Denda Fleksibel",
        features: [
          { name: "Tagihan Rutin Terpadu", desc: "Pengelolaan tagihan IPL (keamanan & kebersihan taman), tagihan pemakaian air meteran m³, dan iuran sampah bulanan" },
          { name: "Tagihan Kustom & Insidental", desc: "Pembuatan tagihan khusus tambahan untuk kegiatan warga, iuran renovasi, atau donasi sarana insidental" },
          { name: "Bulk Generator Tagihan 1-Klik", desc: "Pembuatan tagihan massal untuk seluruh kavling berpenghuni dalam 1 kali klik setiap awal periode penagihan" },
          { name: "Pengaturan Denda Fleksibel (% & Rp)", desc: "Konfigurasi denda keterlambatan dalam persentase (%) atau nominal angka langsung (Rp) disertai simulasi live kalkulasi sebelum disimpan" },
          { name: "Pencatatan Meteran Air Bersih", desc: "Input dinamis angka meter awal, meter akhir, volume kubikasi pemakaian (m³), dan kalkulasi tarif otomatis" },
        ],
      },
      {
        icon: "💳",
        module: "Super Admin: Verifikasi Pembayaran & Laporan Arus Kas",
        features: [
          { name: "Notifikasi Otomatis Gateway Midtrans", desc: "Sinkronisasi pelunasan real-time via Midtrans Webhook Handler untuk transaksi QRIS (GoPay, OVO, Dana, ShopeePay) dan Virtual Account Bank" },
          { name: "Verifikasi Pembayaran Manual & Tunai", desc: "Antrean verifikasi transfer bank dengan preview screenshot bukti bayar, opsi Setujui (Approve) atau Tolak (Reject) disertai catatan verifikasi" },
          { name: "Laporan Arus Kas & Analisis Recharts", desc: "Grafik keuangan interaktif penerimaan iuran IPL, pembukuan pengeluaran operasional, laba/rugi, dan status kolektibilitas iuran warga bulanan" },
        ],
      },
      {
        icon: "📢",
        module: "Super Admin: Broadcast Selektif & Pusat Aspirasi Warga",
        features: [
          { name: "Broadcast Umum & Pesan Khusus Target", desc: "Kirim pengumuman ke seluruh klaster atau kirim pesan peringatan tertuju ke blok/warga tertentu dengan seleksi interaktif klik baris/kartu warga" },
          { name: "Integrasi WhatsApp Fonnte & Nodemailer SMTP", desc: "Pengiriman otomatis via WhatsApp Gateway tanpa footer pihak ketiga serta pengiriman kwitansi digital, notifikasi tagihan jatuh tempo, dan reset password via email" },
          { name: "Manajemen Pengaduan & Aspirasi Warga", desc: "Pelacakan tiket keluhan (kerusakan fasilitas, keamanan, sampah) dengan status bertahap (Baru ➡️ Diproses ➡️ Selesai) dan fitur tanggapan resmi pengelola" },
          { name: "Manajemen Akun & Log Aktivitas (Audit Trail)", desc: "Pembuatan akun warga baru, fitur reset password mandiri/manual ke default 'Warga123!', dan rekam jejak digital aktivitas lengkap dengan waktu dan pelakunya" },
        ],
      },
      {
        icon: "🏡",
        ...
        features: [
          { name: "Dashboard Pribadi & Status Kavling", desc: "Ringkasan kepemilikan unit kavling, notifikasi status tagihan bulan berjalan (Lunas / Belum Lunas), peringatan jatuh tempo, dan pop-up pengumuman penting terbaru" },
          { name: "Pembayaran Tagihan Multi-Metode", desc: "Pembayaran online Midtrans Snap (QRIS & VA Bank BCA/Mandiri/BNI/BRI/Permata) dan pembayaran manual transfer bank dengan fitur upload struk bukti bayar" },
          { name: "Arsip Kwitansi Digital Lunas", desc: "Riwayat kwitansi digital sah berstatus lunas yang dapat diakses dan diunduh sewaktu-waktu sebagai arsip pribadi" },
          { name: "Layanan Pengaduan & Unggah Foto Bukti", desc: "Formulir pelaporan kendala sarana/fasilitas lengkap dengan unggahan foto kerusakan serta pemantauan progres penanganan transparan" },
          { name: "Peta Siteplan Warga & Profil Mandiri", desc: "Tampilan visual denah lingkungan kavling dari sudut pandang warga serta kemudahan pembaruan profil kontak dan ubah password mandiri" },
        ],
      },
      {
        icon: "📱",
        module: "PWA (Progressive Web App) & Mobile Native Feel",
        features: [
          { name: "Web App Manifest Terstandarisasi", desc: "Konfigurasi manifest.json dengan ikon multi-resolusi (192px, 512px, Apple Touch), orientasi potret responsif, dan tema warna #2067A1" },
          { name: "Custom Service Worker (public/sw.js)", desc: "Penerapan strategi caching Network-First untuk halaman navigasi dan Network-Only untuk endpoint transaksi API agar data selalu mutakhir" },
          { name: "Auto-Update Engine Tanpa Re-download APK", desc: "Deteksi otomatis versi rilis baru dengan dialog pop-up ramah 'Perbarui Sekarang' atau 'Nanti' langsung di browser HP/desktop" },
          { name: "Fitur 'Tekan 2x Tombol Back untuk Keluar'", desc: "Meniru perilaku alami aplikasi Android native: 1x tekan memunculkan toast '👋 Tekan sekali lagi untuk keluar', 2x tekan dalam 2 detik langsung menutup/minimize aplikasi" },
        ],
      },
      {
        icon: "🛡️",
        module: "Keamanan Enterprise, Isolasi Sesi & Aksesibilitas Modern",
        features: [
          { name: "Isolasi Dual-Session (Anti-Bentrok Multi-Tab)", desc: "Sesi Admin (gt_admin_session) dan Warga (gt_warga_session) dipisah independen di tingkat cookie dan middleware sehingga admin & warga dapat dibuka di browser yang sama tanpa saling logout" },
          { name: "Auto-Logout 5 Menit Inactivity Timeout", desc: "Otomatis mengunci sesi jika tidak aktif selama 5 menit, dilengkapi dialog santun di menit ke-4 (sisa 60 detik) dengan tombol 'Tetap Masuk', sinkronisasi multi-tab localStorage, dan proteksi sleep mode" },
          { name: "Antarmuka Ramah Orang Tua (Elderly-Friendly UI)", desc: "Tombol 'Keluar Aplikasi' berada tepat di bawah 'Masuk ke Akun' di halaman login, input teks lega (py-3) berdaya kontras tinggi, dan modal konfirmasi keluar 1 baris tombol besar" },
          { name: "Dukungan Tema Gelap & Terang (Dark & Light Mode)", desc: "Toggle tema instan di semua halaman dengan rasio kontras visual terkalibrasi demi kenyamanan mata di segala kondisi cahaya" },
        ],
      },
      {
        icon: "⚡",
        module: "Arsitektur Backend, Dual-Database & Cloud Infra",
        features: [
          { name: "Dual-Database Real-Time Engine", desc: "Kolaborasi Firebase Cloud Firestore (v12.19.0 NoSQL document store) untuk operasional real-time dan Supabase SSR/Client untuk PostgreSQL & storage berkas bukti bayar" },
          { name: "Next.js 15.5 Route Handlers & REST API", desc: "Arsitektur endpoint API modular internal (/api/admin/*, /api/warga/*, /api/auth/*, /api/cron/*) dengan TypeScript 5 Strict Type-Safety" },
          { name: "Cloud Edge Deployment di Vercel", desc: "Hosting Vercel Serverless Edge dengan auto-build branch main GitHub, pemantauan Vercel Analytics real-time, dan 100% kredensial sensitif diamankan di Environment Variables" },
        ],
      },
    ],
  },
