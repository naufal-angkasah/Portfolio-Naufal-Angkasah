...
// Tracked IDs — critical above-the-fold elements
export const READY_IDS = {
  FONTS:     "fonts",      // document.fonts.ready
  DOM:       "dom",        // DOM interactive & initial paint
...
export default function LoadingScreen() {
  ...
  useEffect(() => {
    ...
    // ── 2. DOM Interactive (Fast initial paint) ───────────────────────────
    const onDomReady = () => {
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          {/* Glow backdrop */}
          <div className="absolute h-[380px] w-[380px] rounded-full bg-cyan-500/15 blur-[100px] pointer-events-none" />
          ...
          <motion.div
            ...
            className="loader-panel relative mx-4 max-w-md w-full overflow-hidden rounded-[2.5rem] border border-cyan-300/20 bg-slate-950/75 p-6 text-center shadow-[0_25px_80px_rgba(0,0,0,0.65)] backdrop-blur-2xl md:p-8"
          >
            {/* Badge */}
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-950/60 px-3.5 py-1 text-[11px] font-black uppercase tracking-[0.25em] text-cyan-200 shadow-inner">
            ...
            <div className="relative mx-auto my-2 flex items-center justify-center">
              <div className="relative grid h-20 w-20 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-sky-700 p-1 shadow-[0_0_30px_rgba(34,211,238,0.4),inset_4px_4px_10px_rgba(255,255,255,0.6)]">
                <div className="relative h-full w-full overflow-hidden rounded-[0.95rem] bg-[#05182d] grid place-items-center">
                  <Waves size={30} className="text-cyan-300 animate-pulse" />
                  <span className="absolute inset-0 bg-gradient-to-t from-cyan-500/20 to-transparent" />
                </div>
                <span className="absolute -bottom-1.5 -left-1.5 grid h-6 w-6 place-items-center rounded-lg bg-slate-900 border border-cyan-400/40 text-cyan-200 shadow-md">
                  <Shield size={11} />
                </span>
                <span className="absolute -top-1.5 -right-1.5 grid h-6 w-6 place-items-center rounded-lg bg-slate-900 border border-cyan-400/40 text-cyan-200 shadow-md">
                  <Cpu size={11} />
                </span>
              </div>
            </div>
            ...
            <p className="mt-0.5 text-xs font-semibold uppercase tracking-widest text-cyan-200/70">
            ...
            <div className="mt-5">
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-900/90 border border-white/10 p-0.5">
                <motion.div
